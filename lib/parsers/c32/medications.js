...
Parsers.C32.medications = function (c32) {
  ...
  medications.entries().each(function(entry) {
    ...
    // if we don't have a product name or high-level text,
    // add the originalText version of the product name in
    if (!product_name || !text) {
      el = entry.tag('manufacturedProduct').tag('originalText');
      if (!el.isEmpty()) {
        var productOriginalText = Core.stripWhitespace(el.val());
        if (!product_name) {
          product_name = productOriginalText;
        } else if (!text) {
          text = productOriginalText;
        }
      }
    }
