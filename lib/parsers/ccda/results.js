...
Parsers.CCDA.results = function (ccda) {
  ...
  results.entries().each(function(entry) {
    ...
    // A panel-level date is technically not in the spec, but sometimes it appears
    // anyways. Make sure that if it doesn't appear, we don't accidentally grab
    // the effectiveTime from the first test and display that as a panel-level date
    var panel_date = null;
    el = entry.immediateChildTag('organizer').immediateChildTag('effectiveTime');
    if (!el.isEmpty()) {
      panel_date = parseDate(el.attr('value'));
      if (!panel_date) {
        panel_date = parseDate(el.tag('low').attr('value'));
      }
    }
