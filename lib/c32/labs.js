  ...
  var parse = function (xmlDOM) {
    ...
    for (var i = 0; i < entries.length; i++) {
      ...
      for (var j = 0; j < results.length; j++) {
        ...
        templateTags = result.elsByTag('templateId');
        for (var k = 0; k < templateTags.length; k++) {
          templateTag = templateTags[k];
          // sometimes results organizers contain non-results. we only want results
          if (templateTag.attr('root') == '2.16.840.1.113883.10.20.1.31') {
